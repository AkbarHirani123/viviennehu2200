...
class alipay_service {
  ...
  function arg_sort($array) {
	...
	function create_form_data () {
	  $return = array();
	  $return['gateway'] = $this->gateway.'_input_charset='.$this->parameter['_input_charset'];
	  ...
	  $url = $this->gateway;
		...
		$sort_array = $this->arg_sort($this->parameter);
		while (list ($key, $val) = each ($sort_array)) {
			$return['formdata'][$key] = $this->charset_encode($val,$this->parameter['_input_charset']);
		}
		$return['formdata']['sign'] = $this->mysign;
		$return['formdata']['sign_type'] = $this->sign_type;
